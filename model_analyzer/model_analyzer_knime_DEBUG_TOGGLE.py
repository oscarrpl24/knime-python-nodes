# =============================================================================
# Model Analyzer for KNIME Python Script Node - DEBUG TOGGLE VERSION
# =============================================================================
# This version has a DEBUG_MODE toggle at the top of the script.
# Set DEBUG_MODE = True for extensive logging, False for production use.
#
...
DEBUG_LOGGER = logging.getLogger('ModelAnalyzer_DEBUG_TOGGLE')
...
def debug_log(message: str, level: str = "DEBUG"):
    """Helper function to log debug messages. Only logs if DEBUG_MODE is True."""
    if not DEBUG_MODE:
        return
    if level == "DEBUG":
...
def log_function_entry(func_name: str, **kwargs):
    """Log function entry with parameters. Only logs if DEBUG_MODE is True."""
    if not DEBUG_MODE:
        return
...
def log_function_exit(func_name: str, result=None):
    """Log function exit with return value. Only logs if DEBUG_MODE is True."""
    if not DEBUG_MODE:
        return
...
def log_exception(func_name: str, exception: Exception):
    """Log exception with full stack trace. Only logs if DEBUG_MODE is True."""
    if not DEBUG_MODE:
        return
    debug_log(f"!!! EXCEPTION in {func_name}: {type(exception).__name__}: {exception}", "ERROR")
    debug_log(f"    Stack trace:\n{traceback.format_exc()}", "ERROR")

# Initialization logging (only if DEBUG_MODE is True)
if DEBUG_MODE:
    debug_log("=" * 80)
    debug_log("MODEL ANALYZER DEBUG TOGGLE VERSION INITIALIZED")
...
def create_model_analyzer_app(
    ...
    app_ui = ui.page_fluid(
        ...
        ui.h3("Model Analyzer - DEBUG TOGGLE VERSION"),
        ui.div({"class": "card"},
...
debug_log("=" * 80)
debug_log("MODEL ANALYZER DEBUG TOGGLE VERSION COMPLETED SUCCESSFULLY")
