# =============================================================================
# WOE Editor for KNIME - ADVANCED VERSION - DEBUG
# =============================================================================
...
class BinningConfig:
    """Configuration for advanced binning algorithms - DEBUG VERSION"""
...
def _iv_optimal_get_splits(
    ...
    while len(current_splits) > max_bins - 1:
        if len(current_splits) <= 1:
            break
        current_splits = current_splits[::2]  # Simplified: keep every other split
...
logger.info("=" * 70)
logger.info("WOE EDITOR ADVANCED - DEBUG VERSION")
logger.info("=" * 70)
...
if contains_dv:
    # HEADLESS MODE
    log_progress("=" * 60)
    log_progress("WOE EDITOR ADVANCED - HEADLESS MODE (DEBUG)")
    log_progress("=" * 60)
...
logger.info("=" * 70)
logger.info(f"WOE Editor Advanced (DEBUG) completed at: {datetime.now().isoformat()}")
