...
# 6. TOGGLE: Debug logging can be enabled/disabled at the start of the script
#
# INPUTS:
...
# VERSION: 1.2-TOGGLE-COMMENTATED
# RELEASE DATE: 2026-01-28
# =============================================================================

# =============================================================================
# SECTION 0: DEBUG MODE TOGGLE AND LOGGING INFRASTRUCTURE
# =============================================================================
...
#
# SET DEBUG_MODE TO False TO DISABLE ALL DEBUG LOGGING

# =============================================================================
# DEBUG MODE TOGGLE - SET THIS TO True OR False
# =============================================================================
...
# Configure the debug logger ONLY if DEBUG_MODE is True
# Using getLogger with a name creates a hierarchical logger
DEBUG_LOGGER = None
if DEBUG_MODE:
    DEBUG_LOGGER = logging.getLogger('variable_selection_commentated_toggle')
    ...
    try:
        ...
        file_handler = logging.FileHandler(
            'variable_selection_commentated_toggle_debug.log', 
...
def debug_log(msg, level='debug'):
    ...
    When DEBUG_MODE is False, this function is a no-op.
    
    PARAMETERS:
    - msg (str): The message to log
    - level (str): Log level - 'debug', 'info', 'warning', 'error', 'critical'
    
    RETURNS: None (side effect: logs the message when DEBUG_MODE is True)
    """
    # Early return if debugging is disabled - makes this function a no-op
...
def log_function_call(func):
    ...
    that simply returns the original function unchanged for zero overhead.
    
    PARAMETERS:
    - func: The function to decorate
    
    RETURNS:
    A wrapped function that logs its own execution (when DEBUG_MODE is True)
    Or the original function unchanged (when DEBUG_MODE is False)
    ...
    def my_function(arg1, arg2):
        return result
    """
    # When debugging is disabled, return the function unchanged (zero overhead)
    if not DEBUG_MODE:
        return func
...
# Log the start of the debug session (only when DEBUG_MODE is True)
if DEBUG_MODE:
    debug_log("=" * 80)
    debug_log("TOGGLE VERSION - Variable Selection Node Starting (Commentated)")
...
if DEBUG_MODE:
    debug_log("=" * 80)
    debug_log("DEBUG SESSION COMPLETE (TOGGLE Commentated Version)")
    debug_log(f"Timestamp: {datetime.now().isoformat()}")
    debug_log("=" * 80)
