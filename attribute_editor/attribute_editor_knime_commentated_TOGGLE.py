# =============================================================================
# Attribute Editor for KNIME Python Script Node - TOGGLE DEBUG VERSION (Commentated)
# =============================================================================
...
# Version: 1.4-TOGGLE - Toggle debug version with controllable logging (commentated)
# =============================================================================

# =============================================================================
# DEBUG TOGGLE - Set to True to enable extensive logging, False to disable
# =============================================================================
# Change this single boolean to control all debug logging throughout the script.
# When True: Detailed logging of every function call, parameters, and results
# When False: Silent operation with no debug output (production mode)
DEBUG_ENABLED = True
# =============================================================================
...
# All logging is conditional on the DEBUG_ENABLED flag.
...
# Configure the logger based on DEBUG_ENABLED flag
if DEBUG_ENABLED:
    ...
    # When DEBUG_ENABLED is False, disable all logging
    # Setting level to CRITICAL + 1 effectively disables all log messages
...
def log_function_entry(func_name: str, **kwargs):
    ...
    """
    # Early return if debugging is disabled - no overhead
...
if DEBUG_ENABLED:
    logger.info("Starting Attribute Editor TOGGLE DEBUG version (commentated)")
...
def create_attribute_editor_app(df: pd.DataFrame, initial_dv: Optional[str] = None):
    ...
    try:
        ...
        title_text = "Attribute Editor (DEBUG MODE - Commentated)" if DEBUG_ENABLED else "Attribute Editor"
        ...
        app_ui = ui.page_fluid(
            ...
            # Only show debug banner if debugging is enabled
            ui.div(
                {"class": "debug-banner"},
                "🔧 DEBUG MODE ENABLED - Extensive logging active"
            ) if DEBUG_ENABLED else ui.div(),
...
if DEBUG_ENABLED:
    logger.info("="*80)
    logger.info("ATTRIBUTE EDITOR TOGGLE DEBUG VERSION (COMMENTATED) - MAIN EXECUTION STARTING")
...
if DEBUG_ENABLED:
    logger.info("="*80)
    logger.info("ATTRIBUTE EDITOR TOGGLE DEBUG VERSION (COMMENTATED) - EXECUTION COMPLETE")
