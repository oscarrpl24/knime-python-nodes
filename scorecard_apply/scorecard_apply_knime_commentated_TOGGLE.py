...
# 1.2-DEBUG (2026-01-28): Debug version with extensive logging on every function
...
# Import logging for DEBUG version - provides hierarchical logging system
import logging
...
# =============================================================================
# DEBUG LOGGING SETUP (Conditional based on DEBUG_MODE)
# =============================================================================
# This section configures the logging system for comprehensive debug output.
# The logging module provides a flexible framework for emitting log messages.
# When DEBUG_MODE is False, logging is effectively disabled.
# =============================================================================

if DEBUG_MODE:
    # Configure the root logger with debug level and detailed format
    ...
    # Create a named logger specifically for this script
    # This allows filtering of messages from this script vs other loggers
    logger = logging.getLogger('ScorecardApply_Commentated_TOGGLE')
    logger.setLevel(logging.DEBUG)
else:
    # Create a null logger that does nothing (minimal overhead)
...
def debug_print(*args, **kwargs):
    ...
    Use this for debug-specific console output.
    """
    if DEBUG_MODE:
        print(*args, **kwargs)
...
def log_function_call(func):
    """
    Decorator that logs comprehensive debug information for function calls.
    
    When DEBUG_MODE is True, this decorator wraps any function to automatically log:
    - Function entry with all parameter values
    - Parameter types and DataFrame shapes
    - Return value with type and shape information
    - Execution timing in seconds
    - Exception details with full traceback
    
    When DEBUG_MODE is False, the decorator simply runs the function with no overhead.
    ...
    Returns:
        wrapper: A wrapped function that includes conditional logging
    """
    @functools.wraps(func)  # Preserves function metadata (name, docstring, etc.)
    def wrapper(*args, **kwargs):
        # If debug mode is off, just run the function without any logging overhead
        if not DEBUG_MODE:
        ...
        # Log each positional argument with type-specific formatting
        for i, arg in enumerate(args):
            if isinstance(arg, pd.DataFrame):
                # For DataFrames, log shape, columns, and a preview
                logger.debug(f"  arg[{i}] (DataFrame): shape={arg.shape}")
                logger.debug(f"    columns={list(arg.columns)[:10]}{'...' if len(arg.columns) > 10 else ''}")
                ...
                # For sequences, log length and first few items
                logger.debug(f"  arg[{i}] ({type(arg).__name__}): len={len(arg)}")
                logger.debug(f"    values={arg[:5]}{'...' if len(arg) > 5 else ''}")
            elif isinstance(arg, dict):
                # For dicts, log keys
                logger.debug(f"  arg[{i}] (dict): keys={list(arg.keys())[:5]}{'...' if len(arg) > 5 else ''}")
            else:
                # For other types, log repr (truncated to 200 chars)
        ...
        try:
            ...
            if isinstance(result, pd.DataFrame):
                ...
                logger.debug(f"  return (DataFrame): shape={result.shape}")
                logger.debug(f"    columns={list(result.columns)[:10]}{'...' if len(result.columns) > 10 else ''}")
                if not result.empty:
...
def is_interaction_term(var_name: str) -> bool:
    ...
    """
    # Log the incoming variable name for debugging (only if DEBUG_MODE)
    if DEBUG_MODE:
        logger.debug(f"Checking if '{var_name}' contains '_x_' pattern")
    ...
    # Log the result (only if DEBUG_MODE)
    if DEBUG_MODE:
...
def parse_interaction_term(var_name: str) -> Tuple[str, str]:
    ...
    if clean_name.startswith('WOE_'):
        ...
        if DEBUG_MODE:
            logger.debug(f"  Removed leading 'WOE_' prefix: '{clean_name}'")
    ...
    if '_x_WOE_' in clean_name:
        ...
        if DEBUG_MODE:
            logger.debug(f"  Split on '_x_WOE_' pattern: {parts}")
    ...
    if '_x_' in clean_name:
        ...
        if DEBUG_MODE:
            logger.debug(f"  Split on '_x_' pattern: {parts}")
        # parts[0] = first variable name
        ...
        if var2.startswith('WOE_'):
            ...
            if DEBUG_MODE:
                logger.debug(f"  Removed 'WOE_' prefix from var2: '{var2}'")
    ...
    logger.error(f"Cannot parse interaction term: {var_name} - no '_x_' pattern found")
...
def create_interaction_bin_label(var1: str, bin1: str, var2: str, bin2: str) -> str:
    ...
    """
    if DEBUG_MODE:
        logger.debug(f"Creating interaction bin label from:")
        logger.debug(f"  var1='{var1}', bin1='{bin1}'")
        logger.debug(f"  var2='{var2}', bin2='{bin2}'")
...
def bin_value_num(cardx: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    if DEBUG_MODE:
        logger.debug(f"bin_value_num called")
        logger.debug(f"  Input cardx shape: {cardx.shape}")
        logger.debug(f"  Input cardx columns: {list(cardx.columns)}")
    
    # Create a copy to avoid modifying the original DataFrame
    # This prevents SettingWithCopyWarning and unintended side effects
    cardx = cardx.copy()
    if DEBUG_MODE:
        logger.debug(f"  Created copy of cardx")
    ...
    if DEBUG_MODE:
        logger.debug(f"  Created bins_value column: {cardx['bins_value'].tolist()[:5]}...")
    ...
    if DEBUG_MODE:
        logger.debug(f"  After regex cleaning: {cardx['bins_value'].tolist()[:5]}...")
    ...
    if DEBUG_MODE:
        logger.debug(f"  Split into {split_values.shape[1]} columns")
    ...
    if split_values.shape[1] >= 1:
        ...
        if DEBUG_MODE:
            logger.debug(f"  Assigned min column: {cardx['min'].tolist()[:5]}...")
    else:
        cardx['min'] = None  # No values to split
        if DEBUG_MODE:
            logger.debug(f"  No values for min column")
    ...
    if split_values.shape[1] >= 2:
        ...
        if DEBUG_MODE:
            logger.debug(f"  Assigned max column: {cardx['max'].tolist()[:5]}...")
    else:
        cardx['max'] = None  # Only one value was present
        if DEBUG_MODE:
            logger.debug(f"  No values for max column")
    ...
    if split_values.shape[1] >= 3:
        ...
        if DEBUG_MODE:
            logger.debug(f"  Assigned is_na column: {cardx['is_na'].tolist()[:5]}...")
    else:
        cardx['is_na'] = None  # No NA indicator present
        if DEBUG_MODE:
            logger.debug(f"  No values for is_na column")
    
    # Special handling for the first row (lowest bin)
    # The first bin typically has no lower bound, so we set min to -99999
    if len(cardx) > 0:
        if DEBUG_MODE:
            old_min = cardx.loc[cardx.index[0], 'min']
            old_max = cardx.loc[cardx.index[0], 'max']
...
def scorepoint_ply(card: pd.DataFrame, df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    if DEBUG_MODE:
        logger.debug(f"scorepoint_ply - MAIN SCORING FUNCTION")
        logger.debug(f"  Input card shape: {card.shape}, columns: {list(card.columns)}")
        logger.debug(f"  Input df shape: {df.shape}")
        logger.debug(f"  Input df columns (first 15): {list(df.columns)[:15]}...")
    ...
    if DEBUG_MODE:
        logger.debug(f"  STEP 1: Found {len(b_columns)} binned columns")
        logger.debug(f"    b_columns: {b_columns}")
    
    # Create a copy of just the binned columns to work with
    # This prevents modifications to the original df
    dt = df[b_columns].copy()
    if DEBUG_MODE:
        logger.debug(f"    Created dt (binned cols only) with shape: {dt.shape}")
    ...
    if DEBUG_MODE:
        logger.debug(f"  STEP 2: Initialized result DataFrame")
        logger.debug(f"    Index length: {len(data.index)}")
        logger.debug(f"    First 5 index values: {list(data.index[:5])}")
    ...
    if DEBUG_MODE:
        logger.debug(f"  STEP 3: Found {len(xs)} variables in scorecard (excluding basepoints)")
    ...
    if DEBUG_MODE:
        logger.debug(f"  STEP 4: Categorized variables")
    ...
    if DEBUG_MODE:
        logger.debug(f"  STEP 5: Processing {len(regular_vars)} regular variables")
    ...
    for x_i in regular_vars:
        ...
        b_x_i = f'b_{x_i}'
        if DEBUG_MODE:
            logger.debug(f"      Expected column: '{b_x_i}'")
        ...
        if DEBUG_MODE:
            logger.debug(f"      Scorecard rows for '{x_i}': {len(cardx)}")
        ...
        dtx = dt[[b_x_i]].copy()
        if DEBUG_MODE:
            unique_bins_in_data = dtx[b_x_i].unique()
            logger.debug(f"      Unique bin values in data: {list(unique_bins_in_data)[:10]}...")
        ...
        if DEBUG_MODE:
            logger.debug(f"      Output column name: '{point_col_name}'")
        ...
        if DEBUG_MODE:
            logger.debug(f"      Performing merge on left='{b_x_i}', right='bin'")
        merged = dtx.merge(
        ...
        # Check for null points (unmatched bins)
        null_count = merged['points'].isna().sum()
        if null_count > 0:
            if DEBUG_MODE:
                logger.warning(f"      Found {null_count} unmatched bins (NULL points)")
        else:
            if DEBUG_MODE:
                logger.debug(f"      All bins matched successfully")
        ...
        if DEBUG_MODE:
            logger.debug(f"      Added '{point_col_name}': min={merged['points'].min()}, max={merged['points'].max()}, mean={merged['points'].mean():.2f}")
    ...
    if DEBUG_MODE:
        logger.debug(f"  STEP 6: Processing {len(interaction_vars)} interaction variables")
    ...
    for x_i in interaction_vars:
        ...
        try:
            ...
            if b_var1 not in dt.columns:
                if DEBUG_MODE:
                    logger.warning(f"      Column '{b_var1}' NOT FOUND - skipping interaction")
            ...
            if DEBUG_MODE:
                logger.debug(f"      Created {len(combined_bins)} combined labels")
                logger.debug(f"        Sample labels: {combined_bins.head(3).tolist()}")
            ...
            if DEBUG_MODE:
                logger.debug(f"      Performing merge on 'combined_bin' vs 'bin'")
            merged = dtx.merge(
                cardx,
            ...
            if null_count > 0:
                if DEBUG_MODE:
                    logger.warning(f"      Found {null_count} unmatched interaction bins (NULL points)")
            else:
                if DEBUG_MODE:
                    logger.debug(f"      All interaction bins matched successfully")
            
            # Add the points to the result DataFrame
            data[point_col_name] = merged['points'].values
            if DEBUG_MODE:
                logger.debug(f"      Added '{point_col_name}'")
    ...
    if DEBUG_MODE:
        logger.debug(f"  STEP 7: Getting basepoints")
    ...
    if DEBUG_MODE:
        logger.debug(f"  STEP 8: Calculating total Score")
    ...
    if DEBUG_MODE:
        logger.debug(f"  COMPLETED: Final result shape: {data.shape}")
...
def validate_scorecard(card: pd.DataFrame) -> Tuple[bool, str]:
    ...
    for col in required_cols:
        if col not in card.columns:
            ...
            logger.error(f"  FAILED: Missing required column '{col}'")
    ...
    if card.empty:
        logger.error(f"  FAILED: Scorecard is empty")
        return False, "Scorecard is empty"
    
    if DEBUG_MODE:
        logger.debug(f"  Scorecard has {len(card)} rows")
    ...
    if 'basepoints' not in card['var'].values:
        logger.error(f"  FAILED: Missing 'basepoints' row")
    ...
    if DEBUG_MODE:
        logger.info(f"  PASSED: Scorecard validation successful")
    return True, ""
...
def validate_data(df: pd.DataFrame, card: pd.DataFrame) -> Tuple[bool, List[str]]:
    ...
    if DEBUG_MODE:
        ...
        logger.debug(f"  df shape: {df.shape}")
        logger.debug(f"  df columns (first 20): {list(df.columns)[:20]}...")
        logger.debug(f"  card shape: {card.shape}")
    ...
    if DEBUG_MODE:
        logger.debug(f"  Summary: found={found_count}, missing={len(missing)}")
    if missing and DEBUG_MODE:
        logger.warning(f"  Missing columns: {missing}")
    if DEBUG_MODE:
        logger.debug(f"  has_any: {has_any}")
...
# Print startup banner to KNIME console and logger
if DEBUG_MODE:
    logger.info("=" * 80)
    logger.info("SCORECARD APPLY NODE - COMMENTATED TOGGLE DEBUG VERSION - STARTING")
    logger.info("=" * 80)

print("Scorecard Apply Node (Commentated Version) - Starting...")
if DEBUG_MODE:
...
if DEBUG_MODE:
    ...
    logger.debug(f"  Data dtypes (first 10): {dict(list(df.dtypes.items())[:10])}...")
    logger.debug(f"  Data index: {df.index[:5].tolist()}...")
    if not df.empty:
        logger.debug(f"  Data head(3):\n{df.head(3).to_string()}")
...
b_columns = [col for col in df.columns if col.startswith('b_')]
if DEBUG_MODE:
    logger.debug(f"  All binned columns in data: {b_columns}")
...
if DEBUG_MODE:
    logger.debug("Checking for flow variables...")
...
try:
    ...
    if DEBUG_MODE:
        logger.debug(f"  WithOriginalData flow variable value: {with_org_fv}")
    ...
    if DEBUG_MODE:
        logger.debug(f"  Error reading flow variable: {e}, using default")
    pass
...
if not has_columns:
    logger.error("No matching binned columns found in data - cannot proceed")
...
if DEBUG_MODE:
    logger.info("Starting scorecard application phase...")
print("\nApplying scorecard...")
...
# Log and print the dimensions of the output
if DEBUG_MODE:
...
# Log and print the dimensions of this larger output
if DEBUG_MODE:
    logger.debug(f"  Full output: shape={df_points_dat.shape}")
    logger.debug(f"  Full output columns (first 30): {list(df_points_dat.columns)[:30]}...")
...
if DEBUG_MODE:
    logger.info("Writing output tables to KNIME...")
...
if DEBUG_MODE:
    logger.debug(f"  Writing Output 0 (Points only): shape={df_points.shape}")
...
if DEBUG_MODE:
    logger.debug(f"  Writing Output 1 (Full data): shape={df_points_dat.shape}")
...
if not df_points.empty and 'Score' in df_points.columns:
    ...
    if DEBUG_MODE:
        ...
        logger.debug(f"  Quartiles: 25%={scores.quantile(0.25)}, 50%={scores.quantile(0.50)}, 75%={scores.quantile(0.75)}")
...
#
# TOGGLE DEBUG VERSION NOTES:
# - Set DEBUG_MODE = True at the top of the script to enable full debug logging
# - Set DEBUG_MODE = False for production use with minimal output
# - All function calls are logged with entry/exit, parameters, and return values
# - Timing information is captured for each function
# - Exception tracebacks are fully logged when DEBUG_MODE is True
# =============================================================================
