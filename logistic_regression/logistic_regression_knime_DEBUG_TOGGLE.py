...
# Create logger
logger = logging.getLogger('LogisticRegression_DEBUG_TOGGLE')
...
def log_variable(name: str, value: Any, context: str = ""):
    """Log a variable's value with optional context. Only logs when DEBUG_MODE is True."""
    if not DEBUG_MODE:
        return
    value_repr = _safe_repr(value)
    if context:
...
def log_checkpoint(message: str):
    """Log a checkpoint in the execution flow. Only logs when DEBUG_MODE is True."""
    if not DEBUG_MODE:
        return
    logger.info(f"[CHECKPOINT] {message}")

def log_dataframe_info(df: pd.DataFrame, name: str = "DataFrame"):
    """Log detailed information about a DataFrame. Only logs when DEBUG_MODE is True."""
    if not DEBUG_MODE:
        return
...
# =============================================================================
log_checkpoint("=" * 70)
log_checkpoint("LOGISTIC REGRESSION NODE - DEBUG TOGGLE VERSION - STARTING")
log_checkpoint("=" * 70)

if DEBUG_MODE:
    print("Logistic Regression Node - DEBUG TOGGLE VERSION (DEBUG ON) - Starting...")
else:
...
log_checkpoint("=" * 70)
log_checkpoint("LOGISTIC REGRESSION NODE - DEBUG TOGGLE VERSION - FINISHED")
log_checkpoint("=" * 70)
